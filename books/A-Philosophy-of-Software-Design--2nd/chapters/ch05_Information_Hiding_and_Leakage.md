...
One of the most important techniques for achieving deep modules is information hiding. This technique was first described in a classic paper by David Parnas[1](part0009.xhtml#a2CB). The basic idea is that each module should encapsulate a few pieces of knowledge, which represent design decisions. The knowledge is embedded in the module’s implementation but does not appear in its interface, so it is not visible to other modules.
...
![](image_rsrc2HA.jpg)  Red Flag: Information Leakage![](image_rsrc2HA.jpg)
...
![](image_rsrc2HA.jpg)  Red Flag: Temporal Decomposition![](image_rsrc2HA.jpg)
...
getParameter returns a parameter value as a string. It provides a slightly deeper interface than getParams above; more importantly, it hides the internal representation of parameters. getIntParameter converts the value of a parameter from its string form in the HTTP request to an integer (e.g., the photo_id parameter in [Figure 5.1](part0009.xhtml#a2D4)). This saves the caller from having to request string-to-integer conversion separately, and hides that mechanism from the caller. Additional methods for other data types, such as getDoubleParameter, could be defined if needed. (All of these methods will throw exceptions if the desired parameter doesn’t exist, or if it can’t be converted to the requested type; the exception declarations have been omitted in the code above).
...
![](image_rsrc2HA.jpg)  Red Flag: Overexposure![](image_rsrc2HA.jpg)
...
[1](part0009.xhtml#a2CN)David Parnas, “On the Criteria to be Used in Decomposing Systems into Modules,” Communications of the ACM, December 1972.
