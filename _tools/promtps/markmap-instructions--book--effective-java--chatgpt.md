...
- Direct children of a heading must be all headings OR all `-` items, never mixed
- No orphaned nodes, no empty branches.
- Always end with a `## Key Takeaways` H2 — the chapter's core arguments distilled.
- `## Key Takeaways` can have listed items(only 2 levels at most) if it's really necessary.
...
### 5. Heading purity rule (critical — silent failure): 
- Every heading node must have children of exactly one kind: either sub-headings (##, ###, ####) or `-` list items, never both. 
- ❌ E.G. A ## heading with - bullets followed by ### sub-headings — the bullets get orphaned/dropped at render.
- When a heading needs both grouped detail (bullets) and sub-topics (headings), push the loose bullets down into their own dedicated sub-heading so the parent holds only headings. Example fix:
  wrong:
  ```markdown
  ## Section          
  - loose bullet
  ...
  ```markdown
  ## Section
    ### Overview (or a sensible heading)
    - loose bullet
    ### Subtopic
  ```

### 6. Emoji — only when semantically precise (never decorative; list items only)
| Marker | Meaning |
|--------|---------|
...
- Heading purity verified: Before output, scan every ##/###/####. For each, confirm its direct children are all headings OR all `-` items, never mixed. This is the most common silent render failure — check it last, deliberately.
